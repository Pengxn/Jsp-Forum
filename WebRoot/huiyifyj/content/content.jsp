...
<c:set var="list" scope="session" value="${PostConnect.postQuery()}"/>
					...
					<c:forEach items="${list}" var="list">
						<tr style="height: auto">
							<td class="left-td-top">
								...
								<c:set var="map" scope="session" value="${ViaConnect.query(list[\"userId\"])}"/>
								<!-- 获取map中的属性 -->
								<c:set var="str" scope="session" value="uploda/${map[\"photo\"]}"/>
								<!-- 获取list中的属性 -->
								<c:set var="str2" scope="session" value="uploda_Post/${list[\"photo\"]}"/>
								...
								<c:set var="list_p" scope="session" value="${CommentConnect.CommentQuery(byid)}"/>
								...
								<c:forEach items="${list_p}" var="list_p">
									...
									<c:set var="map2" scope="session" value="${UserConnect.query(userid_others)}"/>
