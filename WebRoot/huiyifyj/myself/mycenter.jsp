...
<div class="u-main">
    ...
    <div class="u-top">
        ...
        <div class="u-myself">
            <div class="left">
                <div class="img">
	                <div>
		                ...
		                <c:if test="${!empty user}">
		                	<!-- 调用ViaConnect.query(int userId)方法获取到该登录用户的头像信息，保存到map中。查询头像信息 -->
		                	<c:set var="map" scope="session" value="${ViaConnect.query(user.getUserId())}"/>
            ...
            <c:if test="${!empty user}">
        		<!-- 调用UserConnect.query(int userId)方法获取到登录用户的个人信息，保存到map中。查询用户信息 -->
        		<c:set var="map" scope="session" value="${UserConnect.query(user.getUserId())}"/>
	            <div class="right">
	                <div class="zuo">
	                    <div>
	                        <p class="p1">
        ...
        <div class="u-botton">
            ...
            <c:if test="${!empty user}">
            	<!-- 调用PostConnect.QueryId(userid)方法获取到该登录用户发过的所有的帖子，保存到lis中。查询该用户发帖信息 -->
            	<c:set var="list" scope="session" value="${PostConnect.QueryId(user.getUserId())}"/>
            	<!-- 遍历list -->
